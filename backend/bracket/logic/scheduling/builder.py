...
async def generate_single_elimination_bracket_from_teams(
    ...
    return await _create_single_elimination_bracket(
        tournament_id, inputs, bracket_size, name, replace_existing
    )


async def generate_single_elimination_bracket_from_slots(
    tournament_id: TournamentId,
    slots: list[TeamId | None],
    name: str | None = None,
    replace_existing: bool = False,
) -> StageItem:
    """
    Create a single-elimination bracket from an explicit slot layout (team id per slot, or
    None for a bye). Used when the organizer has manually arranged the bracket by dragging, so
    the exact placement (including where byes sit) is preserved.
    """
    real_team_count = sum(1 for team_id in slots if team_id is not None)
    bracket_size = len(slots)
    if real_team_count < 2:
        raise HTTPException(
            status_code=400,
            detail="At least 2 teams are required to generate a bracket",
        )
    if bracket_size < 2 or (bracket_size & (bracket_size - 1)) != 0:
        raise HTTPException(
            status_code=400,
            detail="Bracket size (number of slots) must be a power of two",
        )
...
async def _create_single_elimination_bracket(
    tournament_id: TournamentId,
