    ...
    for stage in stages:
        ...
        for stage_item in stage_items:
            ...
            for round_ in sorted(stage_item.rounds, key=lambda r: r.id):
                # Matches within a round have no dependency on each other, so spread them across
                # the available courts (cycling if there are more matches than courts) to run
                # them in parallel. Court assignment resets each round, so later rounds reuse
                # those courts and land after the earlier round in each court's schedule.
                for court_index, match in enumerate(round_.matches):
