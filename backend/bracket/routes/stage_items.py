...
from bracket.logic.scheduling.builder import (
    ...
    generate_single_elimination_bracket_from_slots,
...
class AutoBracketCreateBody(BaseModelORM):
    ...
    # Optional explicit slot layout (team id per slot, or null for a bye), length must be a
    # power of two. Takes precedence over team_ids; used when the bracket was arranged by hand.
    slots: list[TeamId | None] | None = None
...
async def auto_generate_bracket(
    ...
    if body.slots is not None:
        await generate_single_elimination_bracket_from_slots(
            tournament_id, body.slots, body.name, replace_existing=body.replace_existing
        )
        return SuccessResponse()
